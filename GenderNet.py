...
class GenderNet(nn.Module):
    def __init__(self, filters, layers):
        ...
        for i in range(layers):
            setattr(self, "conv_{}".format(i), nn.Conv1d(filters, filters, 3, dilation=1, padding=1))
            setattr(self, "bn_{}".format(i), nn.BatchNorm1d(filters))
...
class AudioDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, training_files, segment_length, sampling_rate, augment=True):
        ...
        with open("./data_files/libri_train_speakers.json") as file:
            self.speakers = sorted(json.load(file))
        with open("./data_files/libri_train_gender.json") as file:
...
class AudioDataset2(torch.utils.data.Dataset):
    ...
    def __init__(self, training_files, segment_length, sampling_rate, augment=True):
        ...
        random.shuffle(self.audio_files)
        self.augment = augment
        with open("./data_files/libri_test_speakers.json") as file:
            self.speakers = sorted(json.load(file))
        with open("./data_files/libri_test_gender.json") as file:
            self.gender = json.load(file)
...
def save_checkpoint(classifier, optimizer, epoch, checkpoint_dir):
    checkpoint_state = {"classifier": classifier.state_dict(), "optimizer": optimizer.state_dict(), "epoch": epoch}
    checkpoint_dir.mkdir(exist_ok=True, parents=True)
    checkpoint_path = checkpoint_dir / "model.ckpt-{}_GenderNet.pt".format(epoch)
...
if __name__ == "__main__":
    ...
    for t in range(epochs):
        ...
        print("\n......Testing model....\n..........\n")
        test(test_loader, model, "models/model.ckpt-90_GenderNet.pt")
